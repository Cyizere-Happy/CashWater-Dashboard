...
export default function DevicesPage() {
    const { isConnected, valveState, sendValveCommand } = useMQTT();
    ...
    // Use the real valve state for the Main Meter if available
    const displayDevices = devices.map(d => {
        if (d.name === 'Kigali Main Meter' && valveState !== null) {
            return { ...d, status: valveState === 'OPEN' ? '.active' : '.cutoff' };
        }
        return d;
    });

    const handleToggleSupply = (deviceName: string, isCutOff: boolean) => {
        // If it's the main meter, use the dedicated sendValveCommand which target control/valve
        if (deviceName === 'Kigali Main Meter') {
            sendValveCommand(isCutOff ? 'CLOSED' : 'OPEN');
        } else {
            // For other simulation devices, we keep the existing logic but update commands to match firmware style
            const command = isCutOff ? 'CLOSED' : 'OPEN';
            // Note: In a real multi-device setup, these would have their own topics in useMQTT
            console.log(`Simulating command ${command} for ${deviceName}`);
        }

        // Optimistic UI update for all devices
